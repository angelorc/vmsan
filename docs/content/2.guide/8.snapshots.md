...
```bash
vmsan snapshot list [vm-id]
```

Pass an optional VM ID to filter snapshots for a specific VM. Use the alias `vmsan snapshot ls` for a shorter command. Add `--json` for structured output.
